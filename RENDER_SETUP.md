...
このアプリケーションは**Docker**を使用してRenderにデプロイされます：

1. **ビルド時（Dockerイメージ作成時）:**
   - `composer install`: PHP依存関係のインストール
   - `npm install`: Node.js依存関係のインストール
   - `npm run build`: フロントエンドアセット（Vite）のビルド

2. **起動時（コンテナ起動時）:**
   - `scripts/startup.sh`が自動実行されます
   - ルートキャッシュ、設定キャッシュの生成
   - データベースマイグレーションの実行
   - Livewireアセットの公開
   - その後、nginx + php-fpmが起動
